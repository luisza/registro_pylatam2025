...
class CharlaDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['schedule']=SpeechSchedule.objects.filter(speech=context['object']).first()
        return context
...
def register_user_to_speech(request, pk):
    ...
    if not schedule:
        error=True
    if error or not inscription:
        messages.error(request, "Lo lamentamos, primero actualiza tus datos y luego procede con el registro")
        return redirect(reverse('index'))
    
    registros = Register_Speech.objects.filter(speech=schedule).count()
    if registros > schedule.room.spaces:
        messages.error(request, "Lo lamentamos no hay espacio disponibles en esta actividad")
        return redirect(reverse('list_charlas')+"?dia=%d"%(dia,))        
    
    registros = Register_Speech.objects.filter(user=request.user)

    for x in registros:
        print(x.speech.speech.title)
        print (x.speech.start_time <= schedule.start_time and x.speech.end_time >= schedule.start_time )
        print (x.speech.start_time <= schedule.end_time and x.speech.end_time >= schedule.end_time )
            
    registros = registros.filter(Q(
        speech__start_time__lte = schedule.start_time,
        speech__end_time__gte = schedule.start_time)|Q(
        speech__start_time__lte = schedule.end_time,
        speech__end_time__gte = schedule.end_time            
            )
    ...
    messages.success(request, "Felicidades ha sido registrado en %s"%(speech.title) )
    return redirect(reverse('list_charlas')+"?dia=%d"%(dia,)) 
...
def desregistrar_charla(request, pk):
    ...
    if registro:
        ...
        messages.error(request, "Registro no encontrado, ¿es usted el usuario correcto?")
    return redirect(reverse('list_charlas')+"?dia=%d"%(dia,)) 