...
setup_version := `python src/ecsl_proj/__init__.py`
current_path := `pwd`
help:
	...
	@echo "dist - print current version of ecsl"
...
build_docker: clean trans
	docker build  -t "registroecsl:$(setup_version)"  .
...
build_docker_selenium:
	docker build -f docker/Dockerfile.selenium -t registroselenium:$(setup_version)  .

run_docker_selenium:
	 docker run --network="host"  -v $(current_path)/src:/ecsl/src  -v $(current_path)/fixtures:/ecsl/fixtures -v $(current_path)/docs:/ecsl/docs  -ti registroecslselenium:$(setup_version) $(run)
